...
import { Download, X, AlertCircle, Loader2, FileText } from 'lucide-react';
import { formatSize } from '../utils/compressor';

const ImageItem = ({ fileEntry, onRemove }) => {
    const { original, compressed, status, error, progress } = fileEntry;
    ...
    const isPdf = original.name.toLowerCase().endsWith('.pdf');
    const isImage = original.type.startsWith('image/');

    const previewUrl = (compressed && isImage) ? URL.createObjectURL(compressed) : (isImage ? URL.createObjectURL(original) : null);

    let thumbnailContent = null;
    if (previewUrl) {
        thumbnailContent = <img src={previewUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />;
    } else {
        if (isPdf) {
            thumbnailContent = <FileText size={28} color="#e52528" />;
        } else {
    ...
    return (
        <div className="glass-panel" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', width: '100%' }}>
                {/* Thumbnail */}
                <div style={{ width: '60px', height: '60px', borderRadius: '8px', overflow: 'hidden', flexShrink: 0, backgroundColor: '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {thumbnailContent}
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    {status === 'compressing' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            {progress && <span style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>{progress}</span>}
                            <Loader2 className="animate-spin" size={20} color="var(--color-text-secondary)" />
                        </div>
                    )}
                    {status === 'error' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#ff3b30' }}>
